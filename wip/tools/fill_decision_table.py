#!/usr/bin/env python3
"""R2M 決策表：依三欄 AI 建議多數決填寫「你的決定」，並產生彩色 HTML。

三欄為 Cursor／Claude／Codex，各自獨立填寫，不互看。規則：

1. 兩個以上強烈建議 → 採用；HTML 決定欄文字白色
2. 兩個以上一般建議 → 採用；HTML 決定欄文字黃色
3. 兩個以上輕鬆建議 → 採用；HTML 決定欄文字綠色

若無任何強度達兩個以上 → 維持待決定。優先序：強烈 > 一般 > 輕鬆（同時滿足取最高）。
「尚未提供」不計票，所以只有一家填寫時不會自動裁決。已寫上的決定不會被覆寫。

由 R2B `wip/tools/fill_decision_table.py` 移植，差異只在編號前綴與預設路徑；
兩個 repo 各自維護一份，不共用 runtime。
"""
...
# R2M 只有單一產品的編號，沒有 R2B／R2O 那種跨產品 XF-* 列
ROW_ID_RE = re.compile(r"^R2M-")
...
def synthesize_decision(strength: str, count: int, option: str | None, ai_cells: list[str]) -> str:
    ...
    return f"**自動採用（{strength}×{count}，{today}）**：{body}"
...
def process_markdown(text: str) -> tuple[str, list[dict]]:
    ...
    while i < len(lines):
        ...
        if (
            len(cells) >= 6
            and "你的決定" in cells[-1]
            and any("Cursor" in c or "Claude" in c or "Codex" in c for c in cells)
        ):
            out.append(line)
            i += 1
            # skip separator
            ...
            while i < len(lines):
                ...
                if row_cells[0].startswith("---") or set(row_cells[0]) <= {"-", " "}:
                    ...
                    i += 1
                    continue
                # 欄序固定為：ID、主題、現況／選項、Cursor、Claude、Codex、你的決定
                if len(row_cells) < 5:
                    out.append(row)
                    i += 1
                    continue
...
def collect_color_map(text: str) -> dict[str, str]:
    ...
    for line in text.splitlines():
        ...
        if len(cells) < 5 or not ROW_ID_RE.match(cells[0]):
            continue
        ai = cells[-4:-1]
